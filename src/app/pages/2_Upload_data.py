...
# Instructions
st.markdown('''
#### It is recommended that the files have an informative and brief name in order to concatenate all csv files. 
To recognize the samples, that name will be added as a new feature column.

### **You can select multiples files to upload:**
''')
st.markdown('''e.g. [ XY_150.csv, XY_90.csv, XY_-20.csv ], where XY refers to the sample itself, 
            and the code refers to the granulometric classification.''')
...
for i in uploaded_files:
    sample = (i.name[:-4])
    tables[sample] = pd.read_csv(i, sep=',')

# processing EDS column

file_list = []
for i in tables.keys():
    if 'EDS Normalised' in tables[i].columns: # if data comes from zeiss mineralogic
        file_list.append(i)
    ...
    else: # receive any kind of numeric data
        file_list.append(i)
        tables[i].insert(loc = 0, column = 'sample', value = i)
